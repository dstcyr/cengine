...
{
    float x, x2, y, y2;
    x = (float)FC_MIN(A.x, B.x);
    y = (float)FC_MIN(A.y, B.y);
    x2 = (float)FC_MAX(A.x + A.w, B.x + B.w);
    y2 = (float)FC_MAX(A.y + A.h, B.y + B.h);
    {
...
static_inline FC_Rect FC_RectIntersect(FC_Rect A, FC_Rect B)
{
    ...
    if (Bmax < Amax)
        Amax = Bmax;
    result.w = Amax - Amin > 0 ? (int)(Amax - Amin) : 0;
    ...
    if (Bmax < Amax)
        Amax = Bmax;
    result.h = Amax - Amin > 0 ? (int)(Amax - Amin) : 0;
